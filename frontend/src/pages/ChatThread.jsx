...
import { getUserById, getPropertyById, addMessage } from '../data/seed'
...
export default function ChatThread() {
  ...
  const [conv, setConv] = useState(() => storage.getConversations().find((c) => c.id === id))
  ...
  const handleSend = () => {
    if (!text.trim()) return
    addMessage(conv.id, user.id, text.trim())
    setConv(storage.getConversations().find((c) => c.id === id))
    setText('')
  }

...
const SOCIAL_LINKS = [
  ...
  },
  {
    ...
    className: 'bg-text text-white hover:bg-black',
    Icon: XIcon,
...
export default function ShareAppCard() {
  ...
  return (
    <div className="rounded-2xl border border-border bg-white p-5 mb-6 lg:mb-0">
      <p className="font-semibold text-sm">{t('shareApp')}</p>
      <p className="text-muted text-xs mt-1 mb-4 leading-relaxed">{t('shareAppSub')}</p>

      <div className="flex flex-wrap gap-3 justify-center lg:justify-start">
        {SOCIAL_LINKS.map(({ id, labelKey, className, Icon, getUrl }) => (
          <a
            key={id}
            ...
            className={`flex h-12 w-12 items-center justify-center rounded-full transition-colors ${className}`}
          >
            <Icon />
          </a>
        ))}
      ...
      <button
        type="button"
        onClick={handleCopy}
        className="mt-4 w-full rounded-full border border-border py-2.5 text-sm font-medium hover:bg-surface transition-colors"
      >
